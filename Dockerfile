...
CMD cd /frontend && echo "Running npm install" && npm install && \
  ...
  UV_THREADPOOL_SIZE=128 NODE_ENV="$APP_ENV" ./node_modules/.bin/webpack-dev-server --display-reasons --display-chunks --loglevel verbose --progress --inline --hot --host 0.0.0.0; \
  fi
